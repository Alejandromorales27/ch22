...
public class banco {
	...
	public static void main(String[] args) {
		...
		System.out.println
		("Bienvenido a los cajeros del banco pepito grillo, porfavor\n"
		...
		for( int i= 0; i<3; i++) {
			if(seleccion == 1) {
				System.out.println("ingresa cantidad a retirar , "
						+ "tu cantidad en este momento es " + cantidadTotal);
				retiro= leer.nextInt();
				if(retiro<=6000 && retiro%50 ==0) {
					System.out.println("gracias por su retiro");
					cantidadTotal=cantidadTotal-retiro;
					break;
				}else{System.out.println("porfavor ingresa cantidad validad, recuerda multipos de 50s");}
			}
		...
		for( int i= 0; i<3; i++) {
			if(seleccion == 2) {
				...
				}else {System.out.println("porfavor pon un numero valido");}
			}
